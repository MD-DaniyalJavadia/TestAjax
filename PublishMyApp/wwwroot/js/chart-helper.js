...
async function renderGroupedBarChart(config) {
    ...
    if (!containerId || !endpoint || !xField || !seriesConfig.length) {
        console.error("renderGroupedBarChart: Missing required parameters");
        return;
    }
    ...
    try {
        ...
        if (!Array.isArray(rawData) || rawData.length === 0) {
            element.innerHTML = `<div class="text-center text-muted">No data available</div>`;
            return;
        }
        ...
        rawData.forEach(item => {
            ...
            seriesConfig.forEach(series => {
                ...
                grouped[key][field] = (grouped[key][field] || 0) + (Number(item[field]) || 0);
            });
        });
        ...
        const series = seriesConfig.map(s => ({
            name: s.name,
            data: categories.map(cat => grouped[cat][s.field] || 0)
        }));

        // 🔹 Destroy old chart if exists
        if (element.__chartInstance) {
            element.__chartInstance.destroy();
        }
        ...
        console.error("Grouped Bar Chart Error:", error);
        element.innerHTML = `<div class="alert alert-danger m-3">Unable to load chart</div>`;
    }
}
...
async function renderRevenueBarChart(config) {
    ...
    if (!containerId || !endpoint || !xField || !yField) {
        console.error("renderRevenueBarChart: Missing required parameters");
        return;
    }
    ...
    try {
        ...
        const categories = rawData[xField] || [];
        const seriesData = rawData[yField] || [];
        ...
        const chartOptions = {
            ...
            tooltip: {
                ...
                shared: false,  // Disable shared tooltip so each bar gets its own tooltip
                intersect: true,  // Show tooltip when hovering directly over a bar
                y: {
                    formatter: function (val) {
                        return valuePrefix + val.toLocaleString() + valueSuffix;
                    }
                ...
                onDatasetHover: {
                    highlightDataSeries: true // Optional: highlight the bar series when hovering
                }
            },
            legend: { show: false }
        };
        ...
        element.innerHTML = `<div class="text-danger small">Chart load failed</div>`;
    }
}
...
async function renderIncomeChart2(config) {
    const {
        chartSelector,
        endpoint,
        xField,
        yFields = [], // array of { field, name }
        title = 'Monthly Revenue',
        xAxisTitle = 'Months',
        height = 280,
        emptyMessage = 'No data available',
    ...
    if (!chartSelector || !endpoint || !xField || !yFields.length) {
        console.error("chartSelector, endpoint, xField and yFields are required");
        return;
    }
    ...
    try {
        ...
        const categories = rawData.map(item => String(item[xField] ?? 'Unknown'));

        const series = yFields.map((y, index) => ({
            name: y.name,
            data: rawData.map(item => Number(item[y.field]) || 0)
        }));

        const chart = new ApexCharts(chartEl, {
            chart: { height, type: 'area', toolbar: { show: false } },
            colors,
            series,
            xaxis: {
                categories,
...
async function renderBubbleChartUniversal(config) {
    const {
        containerId,       // required: div id jahan chart render hoga
        endpoint,          // required: API URL ya JSON data source
        xField,            // required: data me x-axis field
        yField,            // required: data me y-axis field
        zField,            // required: data me bubble size field
        seriesField = null,// optional: group by field
        title = 'Bubble Chart',
        height = 400,
        valuePrefix = '',
        colors = ['#7367F0', '#28C76F', '#EA5455', '#FF9F43', '#1ABC9C'],
        emptyMessage = 'No data available'
    } = config;

    // validate required parameters
    if (!containerId || !endpoint || !xField || !yField || !zField) {
        console.error("renderBubbleChartUniversal: Required parameters missing");
        return;
    }
    ...
    try {
        ...
        let series = [];
        if (seriesField) {
            // group by seriesField
            const grouped = rawData.reduce((acc, item) => {
                const group = item[seriesField] || 'Others';
                if (!acc[group]) acc[group] = [];
                acc[group].push({
                    x: Number(item[xField]) || 0,
                    y: Number(item[yField]) || 0,
                    z: Number(item[zField]) || 5
                });
                return acc;
            }, {});

            const groupKeys = Object.keys(grouped);
            series = groupKeys.map((key, idx) => ({
                name: key,
                data: grouped[key],
        ...
        const options = {
            ...
            series,
            dataLabels: { enabled: false },
            xaxis: { title: { text: xField }, tickAmount: 6 },
            yaxis: { title: { text: yField } },
            tooltip: {
                shared: true,
                intersect: false, // 🔹 important fix
        ...
        return chart; // return chart instance for further manipulation if needed
    } catch (error) {
        console.error("Bubble chart error:", error);
        el.innerHTML = `<div class="text-danger text-center">Chart load failed: ${error.message}</div>`;
    }
}
